...
split2 : Int -> List Float -> List (List Float) -> List (List Float)
split2 period data result =
  if data == [] then
    result
  else
    let
    ...
    in
      split2 period rest (chunk :: result)


split : Int -> List Float -> List (List Float)
split period data =
  split2 period data [] |> List.reverse
...
listUnzip l =
  let
    innerLength =
      ...
      |> List.length
    init =
      List.map (\i -> []) <| List.range 0 innerLength
  in
...
calculateHoltWinters2
    ...
    -> List Float
    -> List Float
calculateHoltWinters2 alpha beta gamma period m it last_st last_bt index data result =
  case data of
    (value::rest) ->
      let
        ...
        it2 =
          if index - period >= 0 then
            ...
            it
        ft = generateForcast st m bt (getWithDefault (index - period + m) it2)
      in
        calculateHoltWinters2 alpha beta gamma period m it2 st bt (index + 1) rest (ft::result)
    [] ->
      result
...
calculateHoltWinters alpha beta gamma period m initTrend seasonal data =
  ...
  in
    calculateHoltWinters2 alpha beta gamma period m it firstObs initTrend 2 restObs []
